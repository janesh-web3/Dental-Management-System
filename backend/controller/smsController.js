  ...
  try {
    ...
    const personalizedMessages = patients
      ...
      .map(patient => {
        ...
        let personalizedMessage = messageContent;
        // Create a variables object with patient-specific data
        ...
        // Replace template variables with patient-specific data
        personalizedMessage = replaceTemplateVariables(personalizedMessage, patientVariables);
    ...
    if (result.validMessages && result.validMessages.length > 0) {
      const validHistoryRecords = result.validMessages.map((validMsg, index) => ({
        ...
        status: validMsg.status || 'sent',
        messageId: validMsg.messageId?.toString(),
        networkProvider: validMsg.network,
        credit: validMsg.credit,
        sentBy: req.user?._id || null,
        templateUsed,
      ...
      try {
        await SMSHistory.insertMany(validHistoryRecords);
      } catch (historyError) {
        console.error('Error saving valid SMS history:', historyError);
      }
    }
    ...
    if (result.invalidMessages && result.invalidMessages.length > 0) {
      ...
      try {
        await SMSHistory.insertMany(invalidHistoryRecords);
      } catch (historyError) {
        console.error('Error saving invalid SMS history:', historyError);
      }
    }
