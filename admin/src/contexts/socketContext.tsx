...
const SocketProvider: React.FC<SocketProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const onConnect = () => {
      ...
      if (role === 'admin' && adminDetails?._id) {
        ...
        console.log(`Joined user channel for doctor: Doctor:${doctorDetails._id}`);
      } else if (role === 'patient' && patientDetails?._id) {
        socketInstance.emit('joinUserChannel', {
          userId: patientDetails._id,
          userType: 'Patient'
        });
        console.log(`Joined user channel for patient: Patient:${patientDetails._id}`);
      }
    };
    ...
    const onConnectError = (error: Error) => {
      ...
      setConnectionAttempts(prev => {
        ...
        if (newAttempts >= MAX_RECONNECTION_ATTEMPTS) {
          toast.error('Failed to connect to the server. Please check your internet connection and refresh the page.');
        } else {
          toast.warning(`Connection attempt ${newAttempts}/${MAX_RECONNECTION_ATTEMPTS}...`);
        }
  ...
  }, [adminDetails, doctorToken, doctorDetails, patientToken, patientDetails, addNotification, getCurrentRole]);
